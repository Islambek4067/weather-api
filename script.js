...
const getInf = async (name) => {
  ...
  const getData = async (longitude , latitude) => {
    const response = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=temperature_2m,showers,wind_speed_10m,relative_humidity_2m,precipitation&models=icon_seamless&current=rain,showers,snowfall,precipitation&timezone=auto&wind_speed_unit=mph&precipitation_unit=inch`,
    );
    ...
    // .split("/").join(", ");
    temperatureBg.insertAdjacentHTML(
      "beforeend",
      `
  ...
  getData(dataS.results[0].longitude , dataS.results[0].latitude);
};
